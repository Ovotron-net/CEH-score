...
export default function Leaderboard() {
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto page-enter">
            ...
            {entries.length >= 1 && (
                <div
                    className={`grid gap-4 mb-8 ${entries.length >= 3 ? 'grid-cols-3' : entries.length === 2 ? 'grid-cols-2 max-w-sm mx-auto' : 'grid-cols-1 max-w-xs mx-auto'}`}>
            ...
            {entries.length === 0 ? (
                ...
                <div className="bg-card border border-border rounded-xl overflow-hidden">
                    <div
                        className="grid grid-cols-[48px_1fr_80px_80px_100px] gap-4 px-5 py-3 border-b border-border text-muted-foreground text-xs font-medium uppercase tracking-wider">
                        <span>Rank</span>
                        <span>Domain</span>
                    ...
                    {entries.map(entry => (
                        ...
                        >
                            ...
                            <span
                                className="text-right text-muted-foreground text-sm">{entry.score}/{entry.maxScore}</span>
                            <span
                                className={`text-right text-sm font-medium ${scoreColor(entry.percentage)}`}>{entry.percentage}%</span>
                            <span
                                className="text-right text-muted-foreground text-xs">{format(new Date(entry.date), 'MMM d, yy')}</span>
                        </div>
                    ))}
                </div>
            )}
